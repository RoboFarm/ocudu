...
static void fill_cu_cp_appconfig_expert_execution_section(YAML::Node node, const expert_execution_appconfig& config)
{
  {
...
static void fill_cu_cp_appconfig_remote_control_section(YAML::Node node, const remote_control_appconfig& config)
{
...
static void fill_cu_cp_appconfig_e1ap_section(YAML::Node node, const ocucp::e1ap_appconfig& config)
{
  YAML::Node cu_cp_node   = node["cu_cp"];
  YAML::Node e1ap_node    = cu_cp_node["e1ap"];
  e1ap_node["bind_addrs"] = config.bind_addrs;
  fill_sctp_config_in_yaml_schema(e1ap_node, config.sctp);
}

static void fill_cu_cp_appconfig_f1ap_section(YAML::Node node, const ocucp::f1ap_appconfig& config)
{
  YAML::Node cu_cp_node   = node["cu_cp"];
...
void ocudu::fill_cu_cp_appconfig_in_yaml_schema(YAML::Node& node, const cu_cp_appconfig& config)
{
  ...
  fill_cu_cp_appconfig_expert_execution_section(node["expert_execution"], config.expert_execution_cfg);
  fill_cu_cp_appconfig_remote_control_section(node["remote_control"], config.remote_control_config);
  fill_cu_cp_appconfig_e1ap_section(node, config.e1ap_cfg);
