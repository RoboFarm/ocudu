...
/// Creates the NTN configuration manager of a DU, or nullptr when the configuration carries no NTN cell. The
/// measurement info update handler is left unset: the NTN neighbour cell measurement info is generated by the CU-CP,
/// not by the DU.
static std::unique_ptr<ocudu_ntn::ntn_configuration_manager>
create_du_ntn_configuration_manager(const du_manager_params& params, du_configurator& du_cfgtr)
{
  if (not params.ntn.cfg.has_value() or params.ntn.cfg->cells.empty()) {
    return nullptr;
  }

  ocudu_ntn::ntn_configuration_manager_dependencies ntn_deps{
      std::make_unique<du_high_ntn_sib19_update_handler_impl>(du_cfgtr),
      std::make_unique<du_high_ntn_time_provider_impl>(params.mac.mgr.get_subframe_time_mapper()),
      /*meas_info_update_handler=*/nullptr,
      params.ntn.doppler_handler,
      params.services.timers,
      params.services.du_mng_exec};

  return ocudu_ntn::create_ntn_configuration_manager(*params.ntn.cfg, std::move(ntn_deps));
}
...
du_manager_impl::du_manager_impl(const du_manager_params& params_) :
  ...
  ntn_config_manager(create_du_ntn_configuration_manager(params, *this)),
  controller(proc_ctxt, main_ctrl_loop, ntn_config_manager.get())
{
}
