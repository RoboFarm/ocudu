...
{
  // Create the NTN configuration manager when the configuration carries NTN cells. The measurement info update handler
  // is left unset: the NTN neighbour cell measurement info is generated by the CU-CP, not by the DU.
  if (params.ntn.cfg.has_value() and not params.ntn.cfg->cells.empty()) {
    ocudu_ntn::ntn_configuration_manager_dependencies ntn_deps{
        create_ntn_sib19_update_handler(*this),
        create_du_high_ntn_time_provider(params.mac.mgr.get_subframe_time_mapper()),
        /*meas_info_update_handler=*/nullptr,
        params.ntn.doppler_handler,
        params.services.timers,
        params.services.du_mng_exec};

    ntn_config_manager = ocudu_ntn::create_ntn_configuration_manager(*params.ntn.cfg, std::move(ntn_deps));
  }
}
