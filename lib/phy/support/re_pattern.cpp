...
void re_pattern::get_inclusion_mask(bounded_bitset<MAX_NOF_SUBCARRIERS>& mask, unsigned symbol) const
{
  ...
  if ((static_cast<unsigned>(i_crb_highest + 1 - i_crb_lowest) == crb_count) && re_mask.all()) {
    unsigned i_re_begin = i_crb_lowest * NOF_SUBCARRIERS_PER_RB;
    unsigned i_re_end   = std::min(static_cast<unsigned>(NOF_SUBCARRIERS_PER_RB) * (i_crb_highest + 1),
                                 static_cast<unsigned>(mask.size()));
