...
public:
  ...
  filter_type(unsigned nof_rbs, unsigned stride)
  {
    ...
    ocuduvec::sc_prod(rc_filter, rc_filter, 1 / total);
    ocuduvec::sc_prod(tail_correction, tail_correction, total);
                                            ...
                                            unsigned                                     i_layer)
{
  ...
  for (; symbol_index != symbol_index_end; ++symbol_index) {
    ...
    ocudu_assert(layer_dmrs_symbols.empty(),
                               ...
                               port_channel_estimator_fd_smoothing_strategy fd_smoothing_strategy)
{
  ...
  switch (fd_smoothing_strategy) {
    case port_channel_estimator_fd_smoothing_strategy::mean:
      std::fill(
          enlarged_filtered_pilots_out.begin(), enlarged_filtered_pilots_out.end(), ocuduvec::mean(enlarged_pilots_in));
      break;
    case port_channel_estimator_fd_smoothing_strategy::filter: {
      ...
      // Apply filter.
      ocuduvec::convolution_same(enlarged_filtered_pilots, enlarged_pilots, rc.rc_filter);
    } break;

    case port_channel_estimator_fd_smoothing_strategy::none:
    default:
      // No strategy.
      ocuduvec::copy(enlarged_filtered_pilots_out, enlarged_pilots_in);
      break;
  }
}

float ocudu::estimate_time_alignment(const re_measurement<cf_t>&                       pilots_lse,
...
interpolator::configuration ocudu::configure_interpolator(const bounded_bitset<NOF_SUBCARRIERS_PER_RB>& re_mask)
{
  ...
  ocudu_assert(stride != -1, "re_mask seems to have only one active entry.");
...
static void compute_v_pilots(span<cf_t> out, span<const float> in_abs, span<const float> in_arg, bool is_start)
{
  ...
  ocudu_assert(nof_v_pilots < 13, "The maximum number of virtual pilots is 12, requested {}.", nof_v_pilots);
  ocudu_assert(in_abs.size() == nof_v_pilots,
               "Input and output should have the same size, provided {} and {}.",
               in_abs.size(),
               nof_v_pilots);
  ocudu_assert(in_arg.size() == nof_v_pilots,
               "Input and output should have the same size, provided {} and {}.",
               in_arg.size(),
               nof_v_pilots);
  ...
  float mean_abs  = ocuduvec::mean(in_abs);
  float slope_abs = ocuduvec::dot_prod(in_abs, used_indices, 0.0F);
  ...
  float mean_arg  = ocuduvec::mean(in_arg);
  float slope_arg = ocuduvec::dot_prod(in_arg, used_indices, 0.0F);
...
static void add_v_pilots(span<cf_t> new_pilots, span<const cf_t> old_pilots)
{
  ...
  ocudu_assert((nof_v_pilots >= 2) && ((nof_v_pilots & 1U) == 0U),
  ...
  ocuduvec::unwrap_arguments(base_arg, base_pilots);
